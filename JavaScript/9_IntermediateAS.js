...
function orbitalPeriod(arr) {
  ...
  const getOrbitalP=(obj)=>{
    const numerator=Math.pow(earthRadius+obj.avgAlt,3);
    const sqrt=Math.sqrt(numerator/GM);
    const orbP=Math.round(two_pi*sqrt);
    return {name:obj.name,orbitalPeriod:orbP};
  }

  for(let i=0;i<arr.length;i++){
    newarr.push(getOrbitalP(arr[i]));
  }
